<template>
  ...
  >
    <div class="field">
      <label for="ten_dv">Tên tiêu chí</label>
      <span
        style="
          color: #eb5757;
          font-size: 16px;
...
const namePage = "AssessmentCriteria";
...
//-------------------- Start CRUD ------------------
const header_update = ref("SỬA TIÊU CHÍ ĐÁNH GIÁ NHÀ CUNG CẤP");
const header_new = ref("THÊM MỚI TIÊU CHÍ ĐÁNH GIÁ NHÀ CUNG CẤP");
...
const getError = () => {
  if (!dataEdit.value.OecDesc.trim() || !dataEdit.value.OecName.trim()) {
    return true;
  }
  return false;
};
