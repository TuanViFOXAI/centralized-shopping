        ...
        <div class="card p-0">
          <div class="panel-content" style="position: relative" v-if="dataTable">
            <DataTable
              ...
              s
              @rowSelect="openDetailWindown($event)"
              :pt="{
                ...
                <template #body="slotProps">
                  {{
                    slotProps.data.ApprovalAt
                      ? format(slotProps.data.ApprovalAt, "dd/mm/yy")
                      : ""
                  }}
                </template>
              </Column>
      ...
      <TabView>
        <TabPanel header="Danh sách người phê duyệt">
          ...
          >
            ...
            <Column header="Tên cấp phê duyệt" field="WstName"></Column>
...
const Filter = () => {
  ...
  const key =
    ...
    user_create_from +
    user_create_to;
  API.get_all("owdd/getall?page=0&size=0" + key).then((res) => {
...
const openDetailWindown = (event) => {
  ...
  setTimeout(function () {
    API.get_all("owdd/getbyid?id=" + event.data.ID).then((res) => {
      dataEdit.value.detal = res.data;
      dataEdit.value.loading = true;
      dataEdit.value.rowSelect = event.data;
      rowClass(event.data);
    });
  }, 500);
};
