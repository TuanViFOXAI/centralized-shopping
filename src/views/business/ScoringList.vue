<template>
  ...
  <template v-if="!visibleDetailWindow">
    ...
    >
      ...
      <Column header="">
        <template #body="sp">
          <!-- <router-link :to="`scoring-list/${sp.data.ID}`">
            <Button icon="pi pi-pencil" text style="width:32px;height:32px" />
          </router-link> -->
          <Button
            icon="pi pi-info-circle"
            text
            style="width: 32px; height: 32px"
            v-if="sp.data.Status === 'A' && false"
          ></Button>
          <Button
            v-else
            icon="pi pi-pencil"
            text
            style="width: 32px; height: 32px"
            @click="openDetailWindow(sp.data.ID)"
          />
        </template>
      </Column>
    </DataTable>
  ...
  <template v-else-if="scoringMode === 3">
    <div class="card border-raduis-none m-3">
      ...
      <div class="grid">
        <div class="col-8">
          <div class="grid">
            ...
            <div class="col-9">
              ...
              <span class="font-bold text-gray-800">{{ modelState.CmaName }}</span>
            </div>
            <div class="col-3 font-semibold">Ngân sách dự kiến:</div>
            <div class="col-9">
              {{
                getCurrencyLabel(
                  modelState.CMA2.map((x) => x.UnitPrice * x.Quantity).reduce(
                    (a, b) => a + b,
                    0
                  )
                )
            ...
            <div class="flex col-9 align-items-center">
              {{
                modelState.CMA6.map((x) => x.OondName).length > 1
                  ? `${modelState.CMA6.map((x) => x.OondName)[0]}, ...`
                  : modelState.CMA6.map((x) => x.OondName)[0]
              }}
              <Button
                v-if="modelState.CMA6.map((x) => x.OondName).length > 1"
                text
                label="xem thêm"
                style="padding: 0 0 0 3px; height: 20px; color: var(--primary-color)"
                @click="showNNLV($event)"
              ></Button>
            </div>
            <div class="col-3 font-semibold">Nhóm hàng hoá:</div>
            <div class="flex col-9 align-items-center">
              {{
                modelState.CMA7.map((x) => x.GroupItemName).length > 1
                  ? `${modelState.CMA7.map((x) => x.GroupItemName)[0]}, ...`
                  : modelState.CMA7.map((x) => x.GroupItemName)[0]
              }}
              <Button
                v-if="modelState.CMA7.map((x) => x.GroupItemName).length > 1"
                text
                label="xem thêm"
                style="padding: 0 0 0 3px; height: 20px; color: var(--primary-color)"
                @click="showNHH($event)"
        ...
        <div class="col-4">
          <div class="grid">
            ...
            <div class="col-5">
              {{ getTimeLabel(modelState.ResultReleaseTime, "time") }}
            </div>
          </div>
        </div>
      </div>
      ...
      <OverlayPanel ref="opNNLV">
        <div class="card p-2 gap-3 mw-30rem mh-30rem scroll">
          <Chip
            ...
            :key="item"
            :label="item"
            v-for="item in modelState.CMA6.map((x) => x.OondName)"
          ></Chip>
        </div>
      </OverlayPanel>
      <OverlayPanel ref="opNHH">
        <div class="card p-2 gap-3 mw-30rem mh-30rem scroll">
          <Chip
            ...
            :key="item"
            :label="item"
            v-for="item in modelState.CMA7.map((x) => x.GroupItemName)"
          ></Chip>
        </div>
      </OverlayPanel>
      <Divider />
      <DataTable
        ...
        class="mb-3"
        :pt="ptTable3"
        :value="dtKetQuaChamDiem.trung_list"
        scrollable
        scrollHeight="500px"
        v-model:expandedRows="expandedTongKetRows"
      >
        <template #header>
        ...
        <Column
          field="total_point"
          header="Tổng điểm gói thầu"
          style="width: 13rem; text-align: center"
          headerClass="jf-center"
        ></Column>
        <template #expansion="slotProps">
          <DataTable showGridlines :value="slotProps.data.items" size="small" stripedRows>
            ...
            <ColumnGroup type="footer">
              <Row>
                <Column
                  footer="Tổng giá trị gói thầu:"
                  :colspan="7"
                  footerStyle="text-align:left"
                />
                <Column
                  style="text-align: right"
                  :footer="totalCounting(slotProps.data.items, 'total_price')"
                  :colspan="3"
                />
              </Row>
            </ColumnGroup>
      ...
      <DataTable
        :pt="ptTable3"
        :showGridlines="true"
        :value="dtKetQuaChamDiem.truot_list"
        scrollable
        scrollHeight="500px"
      >
        <template #header>
  ...
  <template v-else>
    <div class="px-3">
      <div class="flex justify-between align-items-center pt-3 px-3 mb-3">
        <h5 class="p-title mb-0 mt-0 font-semibold">
          {{ scoringMode === 1 ? "CHẤM ĐIỂM NHÀ CUNG CẤP" : "LỰA CHỌN NHÀ CUNG CẤP" }}
        </h5>
        ...
        <div class="grid">
            ...
            <div class="grid">
              ...
              <div class="col-6">
                ...
                <span class="mb-2" v-else>{{
                  getTimeLabel(modelState.EndTime, "day")
                }}</span>
              </div>
              ...
              <div class="col-6">
                ...
                <span class="mb-2" v-else>{{
                  `${
                    modelState.CMA1.filter((x) => x.Status && x.Status === "S").length
                  }/${modelState.CMA1.length}`
                }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="col-6 p-fluid">
            <div class="field mb-0">
              <label class="p-label"
                >Trọng số giữa các tiêu chí<sup class="p-important">*</sup></label
              >
              <Dropdown
              ...
              <small v-if="errorMessage.t1 && !modelState.Oosw" class="p-error">{{
                errorMessage.t1
              }}</small>
            </div>
          </div>
          <div class="col-3 p-fluid" v-if="tabToShow['Other'] === 'Y'">
            <div class="field mb-0">
              <label class="p-label"
                >Trọng số tiêu chí khác<sup class="p-important">*</sup></label
              >
              <InputNumber
              ...
              />
              <small
                v-if="errorMessage.t2 && modelState.trongSo === null"
                class="p-error"
                >{{ errorMessage.t2 }}</small
              >
            </div>
          </div>
        </div>
      ...
      <div v-if="!loadingOcma && scoringMode === 1" class="card p-0 border-raduis-none">
        <TabView v-model:activeIndex="activeIndex" :scrollable="true">
          <TabPanel
            v-if="tabToShow['ImpProgress'] === 'Y'"
            header="Tiến độ thực hiện"
            :pt="ptTabPanel"
          >
            <DataTable
              showGridlines=""
              class="card border-raduis-none p-0"
              v-model:expandedRows="expandedTienDoRows"
              :rowClass="rowClass"
              :value="dtTDTHModel"
              :pt="ptTable1"
            >
          ...
          >
            <DataTable
              showGridlines=""
              class="card border-raduis-none p-0"
              v-model:expandedRows="expandedDieuKhoanTTRows"
              :rowClass="rowClass"
              :value="dtDKTTModel"
              :pt="ptTable2"
            >
                ...
                >
                  ...
                  <Column header="2" style="width: 20rem">
                    <template #body="slotP">
                      ...
                      <span
                        @click="openInfoDKTT(slotP.data.data, $event)"
                        class="text-primary cursor-pointer h-font-bold"
                        v-if="slotP.data.id === 2"
                        >chi tiết <i class="pi pi-info-circle"></i
                      ></span>
                      <OverlayPanel ref="opInfoDKTT">
                        <div class="grid w-30rem">
                          ...
                          <div class="col-6">
                            {{
                              dataToView.DateAdd === 1
                                ? `Ngày hoá đơn (${dataToView.Month} tháng + ${dataToView.Day} ngày)`
                                : `Ngày giao hàng (${dataToView.Month} tháng + ${dataToView.Day} ngày)`
                            }}
                          </div>
                          <div class="col-6 font-bold">Số ngày chênh lệch:</div>
                          <div class="col-6">
                            {{ `${dataToView.NumOfDaysDiff} ngày` }}
                          </div>
                        </div>
                      </OverlayPanel>
                    </template>
          ...
          <TabPanel v-if="tabToShow['Origin'] === 'Y'" header="Xuất xứ" :pt="ptTabPanel">
            <DataTable
              showGridlines
              class="card border-raduis-none p-0"
              v-model:expandedRows="expandedXuatXuRows"
              :rowClass="rowClass"
              :value="dtXuatXuModel"
              :pt="ptTable1"
            >
          ...
          >
            <DataTable
              showGridlines
              class="card border-raduis-none p-0"
              v-model:expandedRows="expandedCSBHRows"
              :rowClass="rowClass"
              :value="dtCSBHModel"
              :pt="ptTable1"
            >
          ...
          >
            <DataTable
              showGridlines=""
              class="card border-raduis-none p-0"
              v-model:expandedRows="expandedBaoGiaRows"
              :rowClass="rowClass"
              :value="dtBaoGiaModel"
              :pt="ptTable1"
            >
          ...
          >
            <DataTable
              showGridlines=""
              class="card border-raduis-none p-0"
              v-model:expandedRows="expandedTieuChiKhacRows"
              :rowClass="rowClass"
              :value="dtTieuChiKhacModel"
              :pt="ptTable2"
            >
              <Column expander style="width: 3rem"> </Column>
              <Column
              ...
              <template #expansion="sp">
                ...
                >
                  ...
                  <Column header="2" style="width: 15rem; text-align: center">
                    <template #body="slotP">
                      <a target="_blank" :href="slotP.data.doc"
                        >Mở liên kết <i class="pi pi-external-link"></i
                      ></a>
                    </template>
                  </Column>
                  <Column
        ...
        <div class="flex justify-end gap-3 cp-1 pt-0">
          ...
          <Button
            :loading="loadingSend"
            :disabled="
              activeIndex > tabCount - 2 &&
              modelState.CMA1.filter((x) => x.Status && x.Status === 'S').length < 1
            "
      ...
      <div v-else-if="!loadingOcma && scoringMode === 2">
        ...
        <DataTable
          :value="dtDiemTongKet"
          class="card border-raduis-none p-0"
          v-model:expandedRows="expandedDiemTongKetRows"
          :rowClass="rowClass"
          :pt="ptTableTotal"
        >
          <Column expander style="width: 3rem" />
          ...
          <template #expansion="sp">
            <DataTable
              v-model:selection="sp.data.Selected"
              selectionMode="single"
              size="small"
              :value="sp.data.suppliers.sort((a, b) => b.total - a.total)"
              showGridlines
            >
              <Column field="sup_name" header="Tên nhà cung cấp"> </Column>
              <Column
              ...
              <Column field="bg" style="width: 7rem; text-align: center">
                ...
                <template #body="sl">
                  {{
                    (sl.data.bg_s =
                      Math.round(sl.data.bg * modelState.Oosw.Score7 * 100) / 100)
                  }}
                </template>
              </Column>
              <Column
                ...
                <template #body="sl">
                  {{
                    (sl.data.total =
                      sl.data.tdth_s +
                      sl.data.dktt_s +
                      (tabToShow["Origin"] ? sl.data.xx_s : 0) +
                      (tabToShow["WartyPolicy"] === "Y" ? sl.data.csbh_s : 0) +
                      sl.data.bg_s +
                      (tabToShow["Other"] === "Y" ? sl.data.tck_s : 0))
                  }}
                </template>
              </Column>
              <Column
              ...
              ></Column>
              <Column
                style="text-align: center; width: 12rem"
                headerStyle="text-align: center"
              >
                <template #header>
                ...
                <template #body="sl">
                  ...
                  <Textarea
                    class="w-full"
                    rows="2"
                    v-if="sp.data.Selected === sl.data"
                    v-model="sl.data.LyDo"
                  ></Textarea>
                </template>
              </Column>
            </DataTable>
...
const openDetailWindow = async (id) => {
  ...
  if (tabToShow["ImpProgress"] === "Y")
    dtTDTHModel.value = modelState.CMA2.map((x) => ({
      ...
      item_unit: x.UomName,
      suppliers: modelState.CMA1.filter((zz) => zz.Status && zz.Status === "S").map(
        (z) => ({
          sup_ID: z.ID,
          sup_name: z.Ocrd.CardName,
          delivery_time: z.WPL1.find((i) => i.Cma2ID === x.ID)
            ? getTimeLabel(z.WPL1.find((i) => i.Cma2ID === x.ID).LeadTime, "date")
            : null,
          wpl1_id: z.WPL1.find((i) => i.Cma2ID === x.ID)
            ? z.WPL1.find((i) => i.Cma2ID === x.ID).ID
            : null,
          auto_score: z.WPL1.find((i) => i.Cma2ID === x.ID)
  ...
  if (tabToShow["TermsOfPay"] === "Y")
    dtDKTTModel.value = modelState.CMA1.filter(
      (zz) => zz.Status && zz.Status === "S"
    ).map((x) => ({
      ...x,
      cma1_id: x.ID,
      wpl2_id: x.WPL2.ID,
      expandRow: [
        {
          id: 1,
          title: "Tạm ứng",
          info: x.WPL2.IsAdvReq ? `Tạm ứng ${x.WPL2.NumAdvReq}%` : "Không",
          auto_score: x.WPL2.Point,
          user_score: modelState.IsSeflPoint === "A" ? x.WPL2.SelfPoint : x.WPL2.Point,
        }, //Tạm ứng
        {
          id: 2,
          title: "Thời hạn thanh toán",
          info: ``,
          auto_score: x.WPL2.Point2,
          user_score: modelState.IsSeflPoint === "A" ? x.WPL2.SelfPoint2 : x.WPL2.Point2,
          data: x.WPL2,
        }, // Thời hạn thanh toán
      ],
    }));
    ...
    dtXuatXuModel.value = modelState.CMA2.map((x) => ({
      ...
      suppliers: modelState.CMA1.filter((zz) => zz.Status && zz.Status === "S")
        .map((z) => ({
          sup_ID: z.ID,
          sup_name: z.Ocrd.CardName,
          origin: z.WPL.find((i) => i.Cma2ID === x.ID)
            ? z.WPL.find((i) => i.Cma2ID === x.ID).OriginName
            : null,
          auto_score: z.WPL.find((i) => i.Cma2ID === x.ID)
    ...
    dtCSBHModel.value = modelState.CMA2.map((x) => ({
      ...
      suppliers: modelState.CMA1.filter((zz) => zz.Status && zz.Status === "S")
        .map((z) => ({
          sup_ID: z.ID,
          sup_name: z.Ocrd.CardName,
          warranty_policy: z.WPL.find((i) => i.Cma2ID === x.ID)
            ? z.WPL.find((i) => i.Cma2ID === x.ID).WarrPolicyName
            : null,
          auto_score: z.WPL.find((i) => i.Cma2ID === x.ID)
    ...
    dtBaoGiaModel.value = modelState.CMA2.map((x) => ({
      ...
      suppliers: modelState.CMA1.filter((zz) => zz.Status && zz.Status === "S")
        .map((z) => ({
          sup_ID: z.ID,
          sup_name: z.Ocrd.CardName,
          sup_quantity: z.WPL.find((i) => i.Cma2ID === x.ID)
            ? z.WPL.find((i) => i.Cma2ID === x.ID).QuanCanBeSupp
            : null,
          sup_price: z.WPL.find((i) => i.Cma2ID === x.ID)
            ? z.WPL.find((i) => i.Cma2ID === x.ID).Price
            : null,
          auto_score: z.WPL.find((i) => i.Cma2ID === x.ID)
  ...
  if (tabToShow["Other"] === "Y")
    // if(tabToShow['TermsOfPay'] === 'Y')
    dtTieuChiKhacModel.value = modelState.CMA1.filter(
      (zz) => zz.Status && zz.Status === "S"
    ).map((x) => ({
      ...x,
      cma1_id: x.ID,
  ...
  if (activeIndex.value < tabCount.value - 1) {
    ...
    for (const ncc of modelState.CMA1.filter((x) => x.Status && x.Status === "S")) {
      var obj = {
        ...
        //Tiến độ thực hiện
        WPL1: dtTDTHModel._rawValue
          .map((x) => x.suppliers.find((i) => i.sup_ID === ncc.ID))
          .map((z) => ({ ID: z.wpl1_id, SelfLeadTimePoint: z.user_score })),

        //Điều khoản thanh toán
        WPL2: {
          ID: dtDKTTModel._rawValue.find((x) => x.ID === ncc.ID).wpl2_id,
          SelfPoint: dtDKTTModel._rawValue.find((x) => x.ID === ncc.ID).expandRow[0]
            .user_score,
          SelfPoint2: dtDKTTModel._rawValue.find((x) => x.ID === ncc.ID).expandRow[1]
            .user_score,
        },
      };
          ...
          dtDiemTongKet.value = modelState.CMA2.map((x) => ({
            ...
            suppliers: modelState.CMA1.filter((zz) => zz.Status && zz.Status === "S").map(
              (z) => {
                var obj = {};
                (obj["LyDo"] = null),
                  (obj["wpl_id"] = z.WPL.find((m) => m.Cma2ID === x.ID).ID);
                obj["sup_ID"] = z.ID;
                obj["so_luong_cung_cap"] = z.WPL.find(
                  (z) => z.Cma2ID === x.ID
                ).QuanCanBeSupp;
                obj["gia_cung_cap"] = z.WPL.find((z) => z.Cma2ID === x.ID).Price;
                obj["sup_name"] = z.Ocrd.CardName;
                obj["tdth"] = dtTDTHModel.value
                  .find((a) => a.item_ID === x.ID)
                  .suppliers.find((a) => a.sup_ID === z.ID).user_score;
                obj["dktt"] =
                  dtDKTTModel.value.find((a) => a.cma1_id === z.ID).expandRow[0]
                    .user_score +
                  dtDKTTModel.value.find((a) => a.cma1_id === z.ID).expandRow[1]
                    .user_score;
                obj["bg"] = dtBaoGiaModel.value
                  .find((a) => a.item_ID === x.ID)
                  .suppliers.find((a) => a.sup_ID === z.ID).user_score;
                obj["delivery_time"] = dtTDTHModel.value
                  .find((a) => a.item_ID === x.ID)
                  .suppliers.find((a) => a.sup_ID === z.ID).delivery_time;
                //--------------------------------------------------------------------------
                if (tabToShow["Origin"] === "Y")
                  obj["xx"] = dtXuatXuModel.value
                    .find((a) => a.item_ID === x.ID)
                    .suppliers.find((a) => a.sup_ID === z.ID).user_score;
                if (tabToShow["WartyPolicy"] === "Y")
                  obj["csbh"] = dtCSBHModel.value
                    .find((a) => a.item_ID === x.ID)
                    .suppliers.find((a) => a.sup_ID === z.ID).user_score;
                if (tabToShow["Other"] === "Y")
                  obj["tck"] = dtTieuChiKhacModel.value
                    .find((a) => a.cma1_id === z.ID)
                    .expandRow.reduce((a, b) => (a.user_score + b ? b.user_score : 0), 0);
                return obj;
              }
            ),
            Selected: null,
...
const onClickKetQua = async () => {
  if (
    dtDiemTongKet.value.filter((x) => x.Selected).length < 1 ||
    dtDiemTongKet.value.filter((x) => x.Selected).length < dtDiemTongKet.value.length
  ) {
    showToast(
      "warn",
      "Cảnh báo",
      `Có ${
        dtDiemTongKet.value.length - dtDiemTongKet.value.filter((x) => x.Selected).length
      } vật tư hàng hoá chưa chọn nhà cung cấp.`
    );
  } else {
    // alert('ok');
...
const tongHopNCCTrungThau = (array) => {
  ...
  for (const item of array) {
    for (const sup of item.suppliers) {
      ...
      NhaThauList[idx].total_point =
        Math.round((NhaThauList[idx].total_point + sup.total) * 100) / 100;
  ...
  return {
    trung_list: NhaThauList.filter((x) => x.items.length > 0),
    truot_list: NhaThauList.filter((x) => x.items.length < 1),
  };
};
...
function getWPL(cma1_id, wpl) {
  ...
  for (let i = 0; i < wpl.length; i++) {
    ...
    // Chính sách bảo hành
    if (tabToShow["WartyPolicy"] === "Y")
      obj["SelfWarrPolicyPoint"] = dtCSBHModel.value
        .find((x) => x.item_ID === wpl[i].Cma2ID)
        .suppliers.find((z) => z.sup_ID === cma1_id).user_score;
    // Xuất xứ
    if (tabToShow["Origin"] === "Y")
      obj["SelfOriginPoint"] = dtXuatXuModel.value
        .find((x) => x.item_ID === wpl[i].Cma2ID)
        .suppliers.find((z) => z.sup_ID === cma1_id).user_score;
    // Báo giá
    obj["SelfPricePoint"] = dtBaoGiaModel.value
      .find((x) => x.item_ID === wpl[i].Cma2ID)
      .suppliers.find((z) => z.sup_ID === cma1_id).user_score;
...
const fetchOcmaData = (page, size) => {
  ...
  API.getAsync("ocma/getall", { page: page, size: size })
    .then((res) => {
...
const getStatusLabel = (status) => {
  switch (status) {
    case scoringStatus.dangthuhien:
      return "Đang thực hiện";
    case scoringStatus.hoanthanh:
      return "Hoàn thành";
    case scoringStatus.huy:
      return "Huy";
    case scoringStatus.thatbai:
      return "Hoàn thành";
    default:
      return "Unknown";
  }
...
const getCurrencyLabel = (value) => {
  if (value) {
    let USDollar = new Intl.NumberFormat("en-VI");
