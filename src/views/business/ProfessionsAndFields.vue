...
const menu = ref();
const OOND_model = reactive({ ...model });
const API_OOND_DATA = ref([]);
...
onMounted(() => {
  ...
  API.get_all("oond/getall").then(
    (res) => {
      //Lấy dữ liệu thành công
      if (res.status >= 200) {
        const data = sortData(res.data.data, "ID");
        API_OOND_DATA.value = data;
        nodeSelected.value = data[0];
        loadedOOND.value = true;
      } else {
      }
    },
    //Lỗi mạng
    (err) => {
      loadedOOND.value = false;
      toast.add({
        severity: "error",
        summary: "Lỗi",
        group: "br",
...
const resetForm = () => {
  Object.assign(OOND_model, model);
};
...
const openAdd = () => {
  ...
  OOND_model.Father = nodeSelected.value.ID;
  dialogLabel.value = {
    Case: 1,
    Title: "Thêm mới ngành nghề lĩnh vực".toUpperCase(),
...
const openUpdate = () => {
  dialogLabel.value = {
    Case: 2,
    Title: "Chỉnh sửa ngành nghề lĩnh vực".toUpperCase(),
    PrimaryButton: i18n.global.t("Button.Save"),
    SecondaryButton: i18n.global.t("Button.Cancel"),
  };
  OOND_model.ID = nodeSelected.value.ID;
  OOND_model.IndCode = nodeSelected.value.IndCode;
  OOND_model.IndName = nodeSelected.value.IndName;
  OOND_model.Father = nodeSelected.value.Father;
  OOND_model.Father_name = nodeSelected.value.Father_name;
  OOND_model.Level = nodeSelected.value.Level;
  OOND_model.Level_name = nodeSelected.value.Level_name;
  OOND_model.IsActive = nodeSelected.value.IsActive;
  showDialog();
  errorMessages.Father = null;
  // console.log(OOND_model)
};
...
const openUpdateChild = (element) => {
  dialogLabel.value = {
    Case: 2,
    Title: i18n.global.t("Professions.edit").toUpperCase(),
    PrimaryButton: i18n.global.t("Button.Save"),
    SecondaryButton: i18n.global.t("Button.Cancel"),
  };
  OOND_model.ID = element.ID;
  OOND_model.IndCode = element.IndCode;
  OOND_model.IndName = element.IndName;
  OOND_model.Father = element.Father;
  OOND_model.FatherName = element.FatherName;
  OOND_model.FatherCode = element.FatherCode;
  OOND_model.Level = element.Level;
  OOND_model.IsActive = element.IsActive;
  showDialog();
  errorMessages.Father = null;
};
...
const checkValidate = () => {
  ...
  isValidate.value =
    OOND_model.IndCode &&
    OOND_model.IndCode.trim() &&
    OOND_model.IndName &&
    OOND_model.IndName.trim() &&
    OOND_model.Father &&
    OOND_model.IsActive &&
    OOND_model.ID != OOND_model.Father;
  // Validation for each field
  if (!OOND_model.IndName || !OOND_model.IndName.trim()) {
    errorMessages.IndName = i18n.global.t("Professions.validate");
  }

  if (!OOND_model.IndCode || !OOND_model.IndCode.trim()) {
    errorMessages.IndCode = i18n.global.t("Professions.validate1");
  }

  if (OOND_model.ID == OOND_model.Father) {
    errorMessages.Father = i18n.global.t("Professions.validate2");
  }
};
    ...
    case 1:
      ...
      if (isValidate.value) {
        let data = {
          data: [
            {
              IndCode: OOND_model.IndCode,
              IndName: OOND_model.IndName,
              Father: OOND_model.Father,
              IsActive: OOND_model.IsActive,
            },
          ],
        };
        addNewNode(data);
        ...
        switch (res.data["data"][0]["CodeErr"]) {
          case StatusCode.success:
            try {
              ...
              API_OOND_DATA.value = addNodeToTreeArray(
                API_OOND_DATA.value,
                nodeToTree.Father,
                nodeToTree
              );
          ...
          case StatusCode.duplicate_key:
            errorMessages.IndCode = `Đã tồn tại mã ${res.data["data"][0]["IndCode"]}`;
...
const updateNode = (_data) => {
  loading.value = true;
  API.update({ data: [_data] }, "oond/update")
    .then((res) => {
      loading.value = false;
      if (res.status === 200) {
...
const deleteSelectedNodes = () => {
  ...
  API.delete({ data: arrayId }, "oond/delete")
    .then((res) => {
      if (res.status === 200) {
        loading.value = false;
        ...
        res.data.data.forEach((iRes) => {
          if (iRes.CodeErr == 0) {
            API_OOND_DATA.value = removeNodeById(API_OOND_DATA.value, iRes.ID);
          } else {
            itemError++;
          }
        });
...
const onchangeFather = (node) => {
  OOND_model.Father = node.value.ID;
};
...
<template>
    ...
    <div class="flex">
      <div class="flex align-items-center mt-0 col-6">
        <h5 class="p-title m-2 p-2 mb-0 mt-0 font-semibold">
          {{ $t("Professions.Header").toUpperCase() }}
        </h5>
      </div>
            ...
            <div class="p-fluid">
              <div class="header-panel p-3">
                <div class="card mb-3 p-3">
                  ...
                  <span v-if="loadedOOND"
                    ><span class="font-bold"> {{ $t("Professions.IndCode") }}: </span>
                    {{ nodeSelected["IndCode"] }}</span
                  >
                  <span class="px-5">|</span>
                  <span v-if="loadedOOND"
                    ><span class="font-bold">{{ $t("Status.Status") }}: </span
                    >{{
                ...
                <div id="main-panel" class="card p-0 .overflow-scroll">
                  <div class="card-header p-3">
                    ...
                    <template v-else>
                      ...
                      >
                        {{ $t("Action.SelectRows", [selectedRows.length]) }}
                  ...
                  <div class="panel-content">
                    <tempalte v-if="loadedOOND">
                      ...
                      >
                        ...
                        <Column
                          field="IndCode"
                          :header="$t('Professions.IndCode')"
                          sortable
                        ></Column>
                        <Column
                          field="IndName"
                          :header="$t('Professions.IndName')"
                          sortable
                        ></Column>
                        <Column
                          field="IsActive"
                        ...
                        <Column :exportable="false" :header="$t('Action.act')">
                    ...
                    <template v-else>
                      ...
                      >
                        ...
                        <Column field="IndName" header="Tên đơn vị" sortable>
                          <template #body> <Skeleton></Skeleton> </template
                        ></Column>
                        <Column field="IndCode" header="Mã đơn vị" sortable>
                          <template #body> <Skeleton></Skeleton> </template
                        ></Column>
      ...
      <div class="card">
        <div class="formgrid grid mr-0">
          <div class="field col-4">
            ...
            <div class="formgrid">
              <InputText
                v-model="OOND_model.IndCode"
                class="w-full"
                type="text"
                :class="{ 'p-invalid': errorMessages.IndCode }"
                aria-describedby="text-error"
              ></InputText>
              <small class="p-error" id="text-error">{{ errorMessages.IndCode }}</small>
            </div>
          </div>
          <div class="field col-8 pr-0">
            <div class="input-header">
              <h6>{{ $t("Professions.IndName") }}</h6>
              <span
                style="
                  color: #eb5757;
                  font-size: 16px;
            ...
            <div class="formgrid">
              <InputText
                v-model="OOND_model.IndName"
                class="w-full"
                type="text"
                :class="{ 'p-invalid': errorMessages.IndName }"
                aria-describedby="text-error"
              ></InputText>
              <small class="p-error" id="text-error">{{ errorMessages.IndName }}</small>
            </div>
          </div>
          <div class="field col-12 pr-0">
            <div class="input-header">
              <h6>{{ $t("Professions.of") }}</h6>
              <span
                style="
                  color: #eb5757;
                  font-size: 16px;
  ...
  <Dialog
    :header="$t('Action.deletion').toUpperCase()"
  ...
  >
    <div class="flex align-items-center justify-content-center">
      ...
      <span>{{ $t("Professions.Delete") }}</span>
    </div>
    <template #footer>
      <Button
        class="p-button-text"
  ...
  <Dialog
    header="Xoá ngành nghề lĩnh vựa"
          ...
          <div class="flex flex-wrap">
            <template v-if="canDeleteRows">
              <span class="font-bold"
                >Bạn có chắc chắn muốn xoá {{ selectedRows.length }} dòng đã chọn?</span
              >
              <span class="pt-2"
            ...
            <template v-else>
              ...
              <span class="pt-2"
                >Không thể xoá cơ ngành nghề lĩnh vực có thành viên con bên trong.</span
              >
            </template>
          </div>
        </div>
