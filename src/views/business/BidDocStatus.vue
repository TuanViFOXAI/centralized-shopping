<template>
  <div class="">
    <div class="flex">
      <div class="flex align-items-center mt-0 col-6">
        <h5 class="p-title m-2 p-2 mb-0 mt-0 font-semibold">TRẠNG THÁI HỒ SƠ THẦU</h5>
      </div>
    </div>
  </div>
          ...
          >
            <template #header>
              <div class="grid bg--surface-card">
                <div class="flex flex-column col-6 mt-2">
                  <h5 class="m-0"></h5>
                ...
                <div class="col-6 flex fl-end mt-2">
                  <Button
                    label="Thêm Mới"
                    icon="pi pi-plus"
                  ...
                  <Button
                    label="Xóa"
                    icon="pi pi-trash"
                    class="p-button-danger mx-3 text-end px-3 mr-0 py-2 font-medium pr-0"
            ...
            >
              <template #body="slotProps">
                <span class="p-column-title">SttName</span>
                {{ slotProps.data.SttName }}
              </template>
            </Column>
            <Column
              field="SttDesc"
            ...
            >
              <template #body="slotProps">
                <span class="p-column-title">SttDesc</span>
                {{ slotProps.data.SttDesc }}
              </template>
            </Column>
            <Column
              field="IsActive"
  ...
  >
    <div class="field">
      ...
      <InputText
        ...
        autofocus
        aria-required="true"
        v-model="dataEdit.SttName"
        :class="{ 'p-invalid': submitted && !dataEdit.SttName }"
      />
      <small class="p-invalid" v-if="submitted && !dataEdit.SttName"
        >Không được bỏ trống tên trạng thái.</small
      >
    </div>
    <div class="field">
      <label>Mô tả</label>
  ...
  >
    <div class="flex align-items-center justify-content-center">
      ...
      <span v-if="dataEdit.SttName"
        >Bạn chắc chắn muốn xóa <b>{{ dataEdit.SttName }}</b> ?</span
      >
      <span v-if="selected_data && !dataEdit.SttName"
...
// Api ---------------------------
const api_add = "oost/create";
const api_update = "oost/update";
const api_get = "oost/getall";
const api_delete = "oost/delete";
// ---------------------------------
...
//-------------------- Start CRUD ------------------
const header_update = ref("SỬA TRẠNG THÁI HỒ SƠ THẦU");
const header_new = ref("THÊM MỚI TRẠNG THÁI HỒ SƠ THẦU");
...
const getError = () => {
  if (!dataEdit.value.SttDesc.trim() || !dataEdit.value.SttName.trim()) {
    return true;
  }
  return false;
};
