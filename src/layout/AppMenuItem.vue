...
onMounted(() => {
  if (
    ...
    localStorage.getItem("activeMenuItem") != null
  )
...
<template>
  <li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
    ...
    <a
      v-if="(!item.Link || item.children) && item.visible !== false"
      :href="item.MnCode == 'menu_1' ? '/' : '#'"
    ...
    >
      ...
      <span class="layout-menuitem-text">{{ $t("menu." + item.MnCode) }}</span>
